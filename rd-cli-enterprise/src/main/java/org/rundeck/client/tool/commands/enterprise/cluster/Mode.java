...
public class Mode extends BaseExtension {
    ...
    public boolean active() throws IOException, InputError {
        return changeMode(ExecutionMode.active, EnterpriseApi::executionModeEnable);
    }


    @CommandLine.Command(description = "Set cluster member execution mode Passive")
    public boolean passive() throws IOException, InputError {
        return changeMode(ExecutionMode.passive, EnterpriseApi::executionModeDisable);
    }
    ...
    boolean changeMode(
        ...
        if (!quiet) {
            getOutput().info(String.format("Setting Execution Mode to %s for cluster member %s...", expected, uuid));
        }

        EnterpriseModeResponse mode = getClient().apiCall((e) -> operation.apply(e, uuid));

        if (!quiet) {
            getOutput().info(String.format("Execution Mode change is %s for cluster member %s:", mode.getStatus(), uuid));
